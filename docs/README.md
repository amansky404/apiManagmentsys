...
| `/private/status` | GET | Telemetry for rate limiting and IP session guard (per IP). | Private |
...
- Display of the current `ROUTE_MODE` and module-aware telemetry.
- Login form that shows the retrieved JWT token and IP-bound session metadata.
- Live cards that visualize rate limiting, warnings, and block status for the current IP via `/private/status`.
